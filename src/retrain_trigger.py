...
MIN_SAMPLES_FOR_TRAIN = int(os.getenv("MIN_SAMPLES_FOR_TRAIN", "10"))
...
def train_model(data_path: str = "data/reference.csv") -> RandomForestClassifier:
    ...
    if len(X) < MIN_SAMPLES_FOR_TRAIN:
        raise ValueError(
            f"Insufficient training samples: {len(X)} < {MIN_SAMPLES_FOR_TRAIN}"
        )
    ...
    if y.nunique() < 2:
        raise ValueError(
            f"Target has insufficient class diversity: {y.unique().tolist()}"
        )
